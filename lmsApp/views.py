...
def student_register(request):
    form = forms.studentregisterforms()
    return render(request, 'studentregisters.html', {'form': form})
def studentregisterdata(request):
    print("hai naresh")
    if request.method == 'POST':
        form = forms.studentregisterforms(request.POST)
        if form.is_valid():
            print('Data is Valid')
            form.save()
...
def studentloginaction(request):
    ...
    if request.method == "POST":
        ...
        try:
            check = models.studentregistermodels.objects.get(student_id=student_id, password=password)
...
def messages_box(request):
    ...
    # database = r"C:\Users\lenovo\Desktop\django_lms\db.sqlite3"
    # con = sqlite3.connect(database)
    # con = con.cursor()
    #
    # con.execute("SELECT id FROM lmsApp_borrow WHERE JULIANDAY(return_date)-JULIANDAY(borrowing_date)>=5")
    # rowss = con.fetchall()
    # rowss=list(rowss)
    # for i in rowss:
    #     # idd=i
    #     if i is not None:
    #         con.execute("SELECT contact FROM lmsApp_students WHERE id==?", i)
    #         mobile_num1= con.fetchall()
    #         for m in mobile_num1:
    #
    #             if m is not None:
    #                 mobile_num=str(m)
    #                 print(mobile_num)
    #                 print(i)
    #
    # return HttpResponse(mobile_num)
    if request.method == 'POST':
        student_id = request.POST.get('student_id')
        cod=[student_id]

        database = r"C:\Users\91773\OneDrive\Desktop\django_lms - Copy\db.sqlite3"
        con = sqlite3.connect(database)
        con = con.cursor()
        for i in cod:
            con.execute("SELECT id FROM lmsApp_students WHERE code==?", (i,))
            rowss = con.fetchall()
        # for k in rowss:
        #     n=k
        ...
        for m in rowss:
            result={}
            con.execute("SELECT borrowing_date,return_date, book_id FROM lmsApp_borrow where student_id==?", m)
            data = con.fetchall()
        print("data",data)
        #return HttpResponse(data)
        return render(request, 'due_books.html', {'data': data})
...
def available__books(request):

    database = r"C:\Users\91773\OneDrive\Desktop\django_lms - Copy\db.sqlite3"
    con = sqlite3.connect(database)
    con = con.cursor()
    con.execute("SELECT isbn, title, description, author, publisher, date_published, status FROM lmsApp_books")
    data = con.fetchall()
    #return HttpResponse(data)
    return render(request, 'availablebooks.html', {'data': data})
#
# def available__books(request):
